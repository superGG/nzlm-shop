	...
	public static enum Type {
		...
		COMMON() {
			...
			public boolean accept(Class<?> solrServerClass) {
				...
				if (HttpSolrClient.class.equals(solrServerClass)) {
					return true;
				}
		...
		UPDATE() {
			@Override
			public boolean accept(Class<?> solrClientClass) {
				if (solrClientClass == null) {
					return false;
				}
				//更新操作最好使用下面的Client
				if (ConcurrentUpdateSolrClient.class.equals(solrClientClass)) {
					return true;
				}
				//查询操作最好使用下面的Client
				if (HttpSolrClient.class.equals(solrClientClass)) {
					return true;
				}
				if (this.accept(solrClientClass.getSuperclass())) {
					return true;
				}
				return false;
			}
		...
		QUERY() {
			...
			public boolean accept(Class<?> solrClientClass) {
				...
				//集群方面的Client
				if (LBHttpSolrClient.class.equals(solrClientClass)) {
					return true;
				}
				if (HttpSolrClient.class.equals(solrClientClass)) {
					return true;
